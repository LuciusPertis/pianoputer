...
def pitchshift2(snd_array, n):
    snd_left = snd_array[0:, 0]
    snd_right = snd_array[0:, 1]

    snd_left = pitchshift(snd_left, n)
    snd_right = pitchshift(snd_right, n)
    
    snd_ps = np.array((snd_left, snd_right)).transpose()
...
def main():
    ...
    fps, sound = wavfile.read('./i.wav')

    tones = range(-25, 25)
    sys.stdout.write('Transponding sound file... ')
    sys.stdout.flush()
    transposed_sounds = [pitchshift2(sound, n) for n in tones]
    print('DONE')
    ...
    while True:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if (key in key_sound.keys()) and (not is_playing[key]):
                key_sound[key].play(fade_ms=400)
                is_playing[key] = True
            ...
            if key in key_sound.keys():
                ...
                if vibra:
                    key_sound[key].fadeout(800)
                else:
                    key_sound[key].fadeout(100)
